resource "aws_security_group" "common" {
  description = "common security group rules for server and agent nodes"
  ...
  # flannel - VXLAN traffic - internal
  ingress {
    from_port   = 8472
    to_port     = 8472
    protocol    = "udp"
    cidr_blocks = [var.vpc_cidr_block]
    description = "flannel - VXLAN traffic - internal"
  }

  # server -> agent kubelet communication - internal
  ingress {
    from_port   = 10250
    to_port     = 10250
    protocol    = "tcp"
    cidr_blocks = [var.vpc_cidr_block]
    description = "server to agent kubelet communication - internal"
  }
